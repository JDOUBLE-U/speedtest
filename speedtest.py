        ...
        def connect(self):
            ...
            if ssl:
                try:
                    ...
                    # ssl.wrap_socket was removed in Python 3.12; prefer a
                    # default context when one is available
...
class LiveMeter(object):
    """Live single-line display for a download or upload test.

    While the test runs it shows a time-based progress bar, a sparkline of
    recent throughput and the current (smoothed) speed. When the test is
    done the line collapses into a summary with the measured average and
    the peak speed seen.
    """
    ...
    ASCII = {
        'sparks': ' .:-=+*#', 'bar': '=', 'head': '>', 'done': 'OK',
    }

    def __init__(self, label, icon, color='36', units=('bit', 1),
                 width=30, stream=None):
        self.stream = stream or sys.stdout
        encoding = (getattr(self.stream, 'encoding', None) or '').lower()
        self.glyphs = self.UNI if 'utf' in encoding else self.ASCII
        self.icon = icon if 'utf' in encoding else label[0]
        self.label = label
    ...
    def _speed(self, bps):
        return '%8.2f M%s/s' % (bps / 1e6 / self.units[1], self.units[0])

    def start(self, get_bytes, duration):
        self._get_bytes = get_bytes
        self._duration = float(duration) or 1.0
        self._t0 = timeit.default_timer()
        self._window = [(self._t0, 0)]
        self._rates = []
        self._last_sample = self._t0
        self.peak = 0.0
        self._stop.clear()
    ...
    def _sample(self):
        ...
        total = self._get_bytes()
        self._window.append((now, total))
        # Keep ~0.75 s of history so the "current" figure doesn't jitter
        while len(self._window) > 2 and now - self._window[0][0] > 0.75:
            self._window.pop(0)
        t_old, b_old = self._window[0]
        current = 0.0
        if now > t_old:
            current = (total - b_old) * 8.0 / (now - t_old)
        if now - self._last_sample >= 0.25:
            self._last_sample = now
            self._rates = (self._rates + [current])[-16:]
            self.peak = max(self.peak, current)
        return now - self._t0, current

    def _sparkline(self):
        sparks = self.glyphs['sparks']
        if not self._rates:
            return ''
        top = max(self._rates) or 1.0
        steps = len(sparks) - 1
        return ''.join(sparks[int(round(r / top * steps))]
                       for r in self._rates)

    def _bar(self, frac):
        bar, head = self.glyphs['bar'], self.glyphs['head']
        filled = int(frac * self.width)
        if filled >= self.width:
            return self._c(self.color_code, bar * self.width)
        return (self._c(self.color_code, bar * filled + head) +
                self._c('2', bar * (self.width - filled - 1)))
    ...
    def _render(self):
        with self._lock:
            if self._stop.is_set():
                return
            elapsed, current = self._sample()
            frac = min(elapsed / self._duration, 1.0)
            self._write(u'\r  %s %-8s %s %5.1fs  %s %s\033[K' % (
                self._c(self.color_code, self.icon),
                self.label,
                self._bar(frac),
                elapsed,
                self._c(self.color_code, self._sparkline().ljust(16)),
                self._c('1', self._speed(current)),
            ))

    def stop(self, final_bps):
        self._stop.set()
        if self._thread:
            self._thread.join()
        with self._lock:
            peak = max(self.peak, final_bps)
            self._write(u'\r  %s %-8s %s   %s   %s\033[K\n' % (
                self._c('32', self.glyphs['done']),
                self.label,
                self._c('1', self._speed(final_bps)),
                self._c('2', 'peak %s' % self._speed(peak).strip()),
                self._c(self.color_code, self._sparkline()),
            ))
...
class SpeedtestResults(object):
    ...
    def __init__(self, download=0, upload=0, ping=0, server=None, client=None,
        ...
        self._share = None
        # utcnow() is deprecated since 3.12; same output format as before
    ...
    def csv(self, delimiter=','):
        ...
        row = [data['server']['id'], data['server']['sponsor'],
               ...
               data['server'].get('d', ''), data['ping'], data['download'],
...
class Speedtest(object):
    ...
    def get_servers(self, servers=None, exclude=None):
        ...
        optionally filtered to servers matching those specified in the
        `servers` argument
        """
        if servers is None:
            servers = []
        ...
        url = 'https://www.speedtest.net/api/embed/vz0azjarf5enop8a/config'
    ...
    def get_best_server(self, servers=None):
        ...
        for server in servers:
            ...
            # Average over the samples actually taken (was a hard-coded /6
            # for 3 samples, which halved the reported ping)
    ...
    def download(self, callback=do_nothing, threads=None, meter=None):
        """Test download speed against speedtest.net

        A ``threads`` value of ``None`` will fall back to those dictated
        by the speedtest.net configuration. ``meter`` is an optional
        ``LiveMeter`` used to display live progress.
        """
        ...
        start = timeit.default_timer()
        if meter:
            meter.start(lambda: sum(t.received for t in started[:]),
                        self.config['length']['download'])
        prod_thread.start()
        cons_thread.start()
        _is_alive = thread_is_alive
        ...
        if meter:
            meter.stop(self.results.download)
    ...
    def upload(self, callback=do_nothing, pre_allocate=True, threads=None,
        ...
        payloads = [r[0].data for r in requests[:request_count]]
        ...
        start = timeit.default_timer()
        if meter:
            meter.start(lambda: sum(p.sent for p in payloads),
                        self.config['length']['upload'])
        prod_thread.start()
        cons_thread.start()
        _is_alive = thread_is_alive
        ...
        if meter:
            meter.stop(self.results.upload)
...
def parse_args():
    ...
    parser.add_argument('--plain', action='store_true', default=False,
                        help='Disable the live progress meter and use the '
                             'classic dotted output')
...
def shell():
    ...
    # Live meter only when a human is watching an interactive terminal
    fancy = (not quiet and not debug and not args.plain and
             sys.stdout.isatty())

    # Don't set a callback if we are running quietly
    if quiet or debug or fancy:
        callback = do_nothing
    else:
    ...
    if args.list:
        ...
        for server in speedtest.servers:
            ...
            line = ('%5s) %s (%s, %s)' % (
                server.get('id', '?'), server.get('sponsor', '?'),
                server.get('name', '?'), server.get('country', '?')))
            try:
                printer(line)
            except IOError:
                e = get_exception()
    ...
    if args.download:
        ...
        if fancy:
            speedtest.download(
                threads=threads,
                meter=LiveMeter('Download', u'\u2193', '36', args.units)
            )
        else:
            printer('Testing download speed', quiet,
                    end=('', '\n')[bool(debug)])
            speedtest.download(callback=callback, threads=threads)
    ...
    if args.upload:
        ...
        if fancy:
            speedtest.upload(
                ...
                threads=threads,
                meter=LiveMeter('Upload', u'\u2191', '35', args.units)
            )
        else:
